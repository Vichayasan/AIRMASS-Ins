...
#define FirmwareVersion "0.0.5"
String current_version = "0.0.5";
...
void splash()
{
  ...
  tft.drawString(ProjectName, xpos, ypos, GFXFF); // Draw the text string in the selected GFX free font
  ...
  tft.drawString(FirmwareVersion, 480, 320, GFXFF);
  ...
  for (int i = 0; i < 240; i++)
  {
    tft.drawString("Waiting for GSM...", xpos, 100, GFXFF);
...
void setUpUI()
{
  ...
  firmwarelabel = ESPUI.addControl(Label, "Firmware", String(FirmwareVersion), Emerald, maintab);
...
void composeJson()
{
  ...
  json.concat(",\"project\":");
  json.concat(ProjectName);
  json.concat(",\"v\":");
  json.concat(FirmwareVersion);
  json.concat("\"}");
  Serial.println(json);
